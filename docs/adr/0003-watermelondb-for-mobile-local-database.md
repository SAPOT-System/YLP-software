...
- **Versioned, additive migrations.** `schemaMigrations()` gives the mobile app automatic, replay-on-start schema evolution (`addColumns`/`createTable` steps). The server reached comparable discipline later, via Alembic applied at deploy time rather than client start (see [ADR 0007](0007-alembic-for-server-migrations.md)).
