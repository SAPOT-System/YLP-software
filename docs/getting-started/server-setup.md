...
| `QA_API_TOKEN` | `app/api/testing.py`, **only** when `ENVIRONMENT=development`, which also mounts the `/testing/*` routes |
...
QA_API_TOKEN=<any shared secret; required while ENVIRONMENT=development>
```
