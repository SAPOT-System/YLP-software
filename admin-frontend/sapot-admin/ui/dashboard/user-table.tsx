...
const UserTable: React.FC<UserTableProps> = ({ data, currentPage, totalPages, onPageChange, refreshData }) => {
  ...
  const handleBanClick = async (e) => {
    e.preventDefault();
    if (!selectedUser?.id || !selectedUser?.username) {
      toast.error("Cannot ban user: None");
      return;
    }
  ...
  const handleUnbanClick = async (e) => {
    e.preventDefault();
    if (!selectedUser?.id || !selectedUser?.username) {
      toast.error("Cannot ban user: None");
      return;
  ...
  return (
	...
	{isBanOpen &&
	  <Modal style=''>
	    <form onSubmit={isBanned ? handleUnbanClick : handleBanClick} className="flex flex-col gap-10">
	      <div className="px-2 py-4">{`This action will ban USER '${selectedUser?.username}.' `}</div>
				{
					!isBanned  &&
					<div>
						<label>Duration(in days): </label>
						<input onChange={(e) => setBanDuration(e.target.value)} placeholder="default is 7" min={1} value={banDuration} type="number" />
					</div>
				}
	      <div className="grid grid-cols-3 gap-2">
		...
		{
			"bg-yellow-600 hover:bg-yellow-500": !isBanned
		})} type="submit">{ selectedUser?.is_banned ? "Unban" : "Ban"} user</button>
	      </div>
	    </form>
	  </Modal>
	}
	  ...
	  <tbody className="divide-y divide-gray-100">
		...
		<tr key={user.id} className="hover:bg-gray-50 transition-colors">
		  ...
		  <td className="px-6 py-5 text-sm">
		    <span className={`px-2 py-1 rounded-full text-xs font-medium ${user.status === 'Active' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'
		    }`}>
		      {user.status}
		    </span>
		  </td>
		  ...
		  <td className="px-6 py-5">
			...
			<Ban onClick={() => openConfirmBan(user)} size={18} />
		      </button>
		    </div>
		  </td>
		</tr>
