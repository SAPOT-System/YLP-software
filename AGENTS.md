...
- **Root**: `package.json` (release tooling only, not a workspace root), `package-lock.json`, `cliff.toml` (changelog generation config for `scripts/release.sh`).
...
- `mobile-app/sapot-mobile-app/pnpm-lock.yaml`, `admin-frontend/sapot-admin/package-lock.json`, root `package-lock.json` — regenerate via the package manager, don't hand-edit.
