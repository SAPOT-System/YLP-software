...
if ! "$rotate_key"; then
  if "$key_exists" && ! "$csr_exists"; then
    log_error "server.key exists but no server.csr - either a key is already in use for this server, or a previous run was interrupted before writing the CSR. If no leaf was ever issued from this key, --force --rotate-key is safe despite its warning."
    exit 1
  fi
  if ! "$key_exists" && "$csr_exists"; then
...
log_info "next steps:"
log_info "1. copy server.csr to the CA laptop via USB"
