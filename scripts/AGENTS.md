...
- `scripts/ca/sign-leaf.sh` must never move under `deploy/scripts/`, and no change to this directory's structure or to `scripts/build-bundle.sh` may create a path by which it — or the CA private key it touches — could reach a server. It runs only on an offline signing laptop with the CA USB stick mounted; it must never be copied into a deployment bundle.
...
- [ ] if `scripts/build-bundle.sh`'s file-copy logic changes, confirm the `sign-leaf.sh` exclusion guard still fires (grep for it / re-run a bundle build and check for its absence)
