...
- The bundle's `systemd/` directory carries exactly four database-maintenance units: the `sapot-db-backup` and `sapot-db-backup-verify` service/timer pairs. They are copied by name rather than by glob. The other units in `deployment-scripts/` describe the bare-metal deployment and must not ship.
