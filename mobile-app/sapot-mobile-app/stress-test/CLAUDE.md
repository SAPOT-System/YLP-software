...
```bash
npm install              # installs deps incl. node-datachannel (native, used only by webrtc mode)
...
npx ts-node src/runner.ts --mode webrtc --config verify-stress-test.config.json   # laptop-only, no phone
...
`NODE_EXTRA_CA_CERTS` is **required** for any run touching the server (`ws`, `both`) — it points Node at the server's CA so HTTPS/WSS auth succeeds. The runner ends with `process.exit(0)` because the mDNS responder and native handles keep the event loop alive otherwise.
...
- **`src/peers/`** — one class per transport, all implementing `BasePeer` (`connect`/`startSending`/`stopSending`/`disconnect`/`getMetrics`).
  ...
  - `lan-peer.ts` — opens a TCP server + advertises via mDNS (`@homebridge/ciao`); orchestrator wires peers in a **ring** over `127.0.0.1`. The phone discovers and connects to these.
  - `ws-peer.ts` — logs in via `/auth/token`, opens a `/ws/` WebSocket, sends chat through the server relay; orchestrator assigns each peer the next peer as its target.
  - `webrtc-peer.ts` — real `RTCPeerConnection` via `node-datachannel`. Even-indexed peers are offerers; signaling is wired **in-process** (`a.sendSignal = b.receiveSignal`). Carries an `RTCDataChannel` (`MSG:`/`ACK:` latency probe) and, when `webrtc.media` is set, synthetic RTP audio (and optional H.264 video) tracks.
...
- `WsPeer`: sends `{ type: 'chat', data: { messageId, ... } }` over the server WS relay; waits for `server-ack` within `ackTimeoutMs` (default 5s); records `dropped` on timeout.
- `WrtcPeer` / `WsSignaledWrtcPeer` / `TcpSignaledWrtcPeer`: sends `MSG:{seq}:{sentAt}` over an `RTCDataChannel` (`chat`); receiver echoes `ACK:{seq}:{sentAt}`; sender measures round-trip latency from `sentAt`.
...
| `ws` | WS chat relay | — | — |
| `lan` | TCP encrypted | — | — |
| `webrtc` | RTCDataChannel | ✓ if `media.type` set | ✓ if `audio-video` |
