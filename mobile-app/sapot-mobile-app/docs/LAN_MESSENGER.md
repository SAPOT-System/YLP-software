...
**Location:** `features/shared/services/discovery-service.ts`
...
**Location:** `features/shared/adapters/tcp-client-adapter.ts`
...
**Location:** `features/shared/adapters/tcp-server-adapter.ts`
...
**Source:** `features/shared/services/connection-service.ts:connectToPeer()`
...
- `features/shared/services/connection-service.ts:sendChatMessage()` (lines 819–849)
- `features/shared/services/webrtc-session-manager.ts:sendChatMessage()` (lines 236–273)
...
- `features/shared/services/webrtc-session-manager.ts:setupWebrtcEvents()` (lines 85–105)
...
**Location:** `features/shared/database/schema.ts` (version 6)
...
**Model:** `features/shared/database/model/Conversation.ts`
...
**Model:** `features/shared/database/model/Message.ts`
...
**Model:** `features/shared/database/model/MessageStatus.ts`
...
**Model:** `features/shared/database/model/Peer.ts`
...
| File | Purpose |
|---|---|
| `features/shared/services/connection-service.ts` | Central facade; TCP/WebRTC orchestration; mode guards |
| `features/shared/services/webrtc-session-manager.ts` | Per-peer WebRTC session management |
| `features/shared/services/discovery-service.ts` | mDNS/Zeroconf peer discovery |
...
| `features/shared/adapters/tcp-client-adapter.ts` | Client-side TCP connection wrapper |
| `features/shared/adapters/tcp-server-adapter.ts` | Server-side TCP listener |
| `features/shared/adapters/webrtc-adapter.ts` | RTCPeerConnection wrapper |
| `features/shared/adapters/zeroconf-adapter.ts` | mDNS/Zeroconf wrapper |
| `features/shared/database/schema.ts` | WatermelonDB schema (v6) |
| `features/shared/database/migrations.ts` | Schema migration steps |
...
| `features/shared/stores/app-mode-store.ts` | Mode enforcement (lan/server/auto) |
| `features/shared/stores/secure-config.ts` | Persistent config (expo-secure-store) |
