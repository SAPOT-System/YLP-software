...
| `debug/` | ~0.9 k | 15 | Developer debug panel (dev/QA-only, gated by `config/debug.ts`); `DebugDbService` provides a WatermelonDB table browser/seeder/reset + JSON export-import over the shared `database` instance; `DebugAuthService` (Auth/Users section) seeds test users and switches roles via `UserService`/`UserStore`, injects/clears a fake JWT via `secure-config`, and drives force-logout/reset via `UserService.logout`/`wipeDatabase`; `FaultInjector` (Offline/Network sections) monkey-patches `.emit("data")`/`.sendMessage()` on the live `TcpClientAdapter`/`TcpServerAdapter`/`WsSignalingAdapter` instances at their construction sites (`ConnectionService.getTcpClientAdapter`, `MainContainer`'s constructor) to inject latency/loss/dup/corruption per transport, is a strict no-op when `IS_DEBUG_ENABLED` is false, and also forces `HealthProvider`'s `online` status and skips `SyncService.syncNow()` for the no-internet/server-down/sync-down offline toggles |
