...
We are Batangas State University - Team Sapot ("Company," "we," "us," "our"), a capstone team at Batangas State University TNEU JPLPC Malvar Campus, M.R. Lat St, Brgy. Poblacion, Municipality of Malvar, Batangas 4233.
We operate the mobile application Sapot (the "App"), as well as any other related products and services that refer or link to these legal terms (the "Legal Terms") (collectively, the "Services").
You can contact us by phone at 09260983691, email at 23-62769@g.batstate-u.edu.ph, or by mail to Batangas State University TNEU JPLPC Malvar Campus, M.R. Lat St. Brgy. Poblacion, Municipality of Malvar, Batangas 4233, Philippines.
These Legal Terms constitute a legally binding agreement made between you, whether personally or on behalf of an entity ("you"), and Batangas State University Team Sapot, concerning your access to and use of the Services. You agree that by accessing the Services, you have read, understood, and agreed to be bound by all of these Legal Terms. IF YOU DO NOT AGREE WITH ALL OF THESE LEGAL TERMS, THEN YOU ARE EXPRESSLY PROHIBITED FROM USING THE SERVICES AND YOU MUST DISCONTINUE USE IMMEDIATELY.
Users will be notified of any updates to the legal terms through in-app notifications and, when applicable, via email. Users will be informed at least 7 days before the updated terms take effect, except in cases where immediate changes are required for security, legal compliance, or system integrity. Continued use of the application after the effective date of the updated terms constitutes acceptance of the revised terms.
...
SMS TEXT MESSAGING
MISCELLANEOUS
ACCOUNT SECURITY AND OTP RESPONSIBILITY
CONTACT US
...
If you wish to make any use of the Services, Content, or Marks other than as set out in this section or elsewhere in our Legal Terms, please address your request to: 23-62769@g.batstate-u.edu.ph. If we ever grant you the permission to post, reproduce, or publicly display any part of our Services or Content, you must identify us as the owners or licensors of the Services, Content, or Marks and ensure that any copyright or proprietary notice appears or is visible on posting, reproducing, or displaying our Content.
...
or use any proprietary information or any of our interfaces or our other intellectual property in the design, development, manufacture, licensing, or distribution of any applications, accessories, or devices for use with the App.
...
14. GOVERNING LAW
These Legal Terms shall be governed by and defined following the laws of the Philippines. Batangas State University - Team Sapot and yourself irrevocably consent that the courts of the Philippines shall have exclusive jurisdiction to resolve any dispute which may arise in connection with these Legal Terms.

15. DISPUTE RESOLUTION
Binding Arbitration Any dispute arising out of or in connection with these Legal Terms, including any question regarding its existence, validity, or termination, shall be referred to and finally resolved by the International Commercial Arbitration Court under the European Arbitration Chamber (Belgium, Brussels, Avenue Louise, 146) according to the Rules of this ICAC, which, as a result of referring to it, is considered as the part of this clause. The number of arbitrators shall be one (1). The seat, or legal place, or arbitration shall be Municipality of Malvar, Philippines. The language of the proceedings shall be English, Filipino. The governing law of these Legal Terms shall be substantive law of the Philippines.
...
Program Description By opting into any LAN Messenger System text messaging program, you expressly consent to receive text messages (SMS) to your mobile number. LAN Messenger System text messages may include: account alerts.
Opting Out OTP messages are transactional and sent only when a user requests authentication. Users cannot opt out of OTP messages as they are required for account security and login verification.
...
Support If you have any questions or need assistance regarding our SMS communications, please reply with the keyword HELP. You can also email us at 23-62769@g.batstate-u.edu.ph or call at 09260983691.
...
24. ACCOUNT SECURITY AND OTP RESPONSIBILITY
Users are solely responsible for maintaining the confidentiality and security of their account credentials and One-Time Password (OTP) codes. The platform shall not be held liable for any unauthorized access resulting from the user's failure to safeguard their account information. Users agree to immediately report any suspected unauthorized use or security breach of their account. The use of OTP is intended solely for authentication purposes and must not be shared with any other individual under any circumstances.

25. CONTACT US
In order to resolve a complaint regarding the Services or to receive further information regarding use of the Services, please contact us at:
Batangas State University - Team Sapot Batangas State University TNEU JPLPC Malvar Campus, M.R. Lat St, Brgy. Poblacion, Municipality of Malvar, Batangas 4233, Philippines Phone: 09260983691 Email: 23-62769@g.batstate-u.edu.ph"""
...
def get_terms_and_conditions():
    return {"content": TERMS_AND_CONDITIONS_TEXT}
...
async def login_for_access_token(
    ...
    # Resolve user first (without password check) for IP gate
    pre_auth_user = get_user(form_data.username, session)

    if pre_auth_user:
        gate = check_and_increment_attempt(session, pre_auth_user.id, client_ip)
        if not gate["allowed"]:
        ...
        # Unknown username — consume budget to prevent enumeration via lockout differential.
        phantom_id = uuid.uuid5(uuid.NAMESPACE_URL, f"phantom:{form_data.username}")
        gate = check_and_increment_attempt(session, phantom_id, client_ip)
        if not gate["allowed"]:
            retry_after = max(0, int((gate["locked_until"] - datetime.now(timezone.utc)).total_seconds()))
            raise HTTPException(
                status_code=429,
...
def create_account(request: Request, user: UserCreate, session: SessionDep, background_tasks: BackgroundTasks):
    if not user.terms_accepted:
        raise HTTPException(
            status_code=400,
            detail={"terms_accepted": "You must accept the Terms & Conditions."}
        )
